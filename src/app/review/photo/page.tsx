...
export default function Photo() {
    ...
    const handleImageClick = (image: string, yearIndex: number, index: number) => {
        dispatch(show(image));
        dispatch(isShow(true));
        dispatch(imageArray(photo));
        dispatch(currentIndex(index, yearIndex));
    };
                ...
                <Content key={yearIndex}>
                    ...
                    <ImgContentWrap>
                        {value.image.map((image, index) => (
                            <TouchableOpacity
                                key={index}
                                onPress={() => handleImageClick(image, yearIndex, index)}
                            >
